...
public class Columns {
	...
	public Column update(int arrayListIndex, int optionIndex) {
		...
		if (optionIndex < mOptions.size()) // No flex customizations added
	...
	protected String generateColumn(Column column, ReceiptRow receipt, TripRow currentTrip) {
		if (column.getColumnType().equals(ColumnName.BLANK)) {
			...
			return (receipt.hasFile()) ? "" : receipt.getFileName();
		}
		else if (column.getColumnType().equals(ColumnName.IMAGE_PATH)) {
			return (receipt.hasFile()) ? "" : receipt.getFilePath();
		}
	...
	public ArrayList<CharSequence> generateOptionsList(Context context, Flex flex) {
		ArrayList<CharSequence> options = new ArrayList<CharSequence>(16);
		...
		if (!TextUtils.isEmpty(ColumnName.EXTRA_EDITTEXT_1)) options.add(ColumnName.EXTRA_EDITTEXT_1);
		if (!TextUtils.isEmpty(ColumnName.EXTRA_EDITTEXT_2)) options.add(ColumnName.EXTRA_EDITTEXT_2);
		if (!TextUtils.isEmpty(ColumnName.EXTRA_EDITTEXT_3)) options.add(ColumnName.EXTRA_EDITTEXT_3);
		return options;
	}
