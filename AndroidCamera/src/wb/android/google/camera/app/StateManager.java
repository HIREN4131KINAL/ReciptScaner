...
import wb.android.google.camera.BuildConfig;
...
public class StateManager {
    ...
    public void startState(Class<? extends ActivityState> klass,
            Bundle data) {
        if (BuildConfig.DEBUG) Log.v(TAG, "startState " + klass);
    ...
    public void startStateForResult(Class<? extends ActivityState> klass,
        ...
        if (BuildConfig.DEBUG) Log.v(TAG, "startStateForResult " + klass + ", " + requestCode);
        ActivityState state = null;
        try {
            state = klass.newInstance();
        } catch (Exception e) {
    ...
    void finishState(ActivityState state, boolean fireOnPause) {
        ...
        if (mStack.size() == 1) {
            ...
            if (!activity.isFinishing()) {
                if (BuildConfig.DEBUG) Log.w(TAG, "finish is rejected, keep the last state");
                return;
            }
            if (BuildConfig.DEBUG) Log.v(TAG, "no more state, finish activity");
        }

        if (BuildConfig.DEBUG) Log.v(TAG, "finishState " + state);
        if (state != mStack.peek().activityState) {
            if (state.isDestroyed()) {
                if (BuildConfig.DEBUG) Log.d(TAG, "The state is already destroyed");
                return;
            } else {
    ...
    public void switchState(ActivityState oldState,
        ...
        if (BuildConfig.DEBUG) Log.v(TAG, "switchState " + oldState + ", " + klass);
    ...
    public void destroy() {
        if (BuildConfig.DEBUG) Log.v(TAG, "destroy");
    ...
    public void restoreFromState(Bundle inState) {
        if (BuildConfig.DEBUG) Log.v(TAG, "restoreFromState");
        ...
        for (Parcelable parcelable : list) {
            ...
            try {
                if (BuildConfig.DEBUG) Log.v(TAG, "restoreFromState " + klass);
    ...
    public void saveState(Bundle outState) {
        if (BuildConfig.DEBUG) Log.v(TAG, "saveState");
        ...
        for (StateEntry entry : mStack) {
            ...
            if (BuildConfig.DEBUG) Log.v(TAG, "saveState " + entry.activityState.getClass());
            list[i++] = bundle;
        }
