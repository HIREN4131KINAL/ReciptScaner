...
public final class DatabaseHelper extends SQLiteOpenHelper implements AutoCompleteQueriable {
	...
	public interface TripRowListener {
		...
		public void onTripRowInsertFailure(SQLException ex, File directory); //Directory here is out of date
	...
	public interface ReceiptRowListener {
		...
		public void onReceiptRowInsertFailure(SQLException ex); //Directory here is out of date
	...
	public final void onUpgrade(final SQLiteDatabase db, int oldVersion, final int newVersion) {
		synchronized (mDatabaseLock) {
			...
			File sdDB = _activity.getStorageManager().getFile(DATABASE_NAME + "." + oldVersion + ".bak");
			try {
				_activity.getStorageManager().copy(new File(db.getPath()), sdDB, true);
			...
			if (oldVersion == 1) { // Add currency column to receipts table
				final String alterReceipts = "ALTER TABLE " + ReceiptsTable.TABLE_NAME 
						...
						+ "DEFAULT " + _activity.getPreferences().getDefaultCurreny();
			...
			if (oldVersion == 6) { //Fix the database to replace absolute paths with relative ones
				...
				if (tripsCursor != null && tripsCursor.moveToFirst()) {
					...
					do {
						...
						if (D) Log.d(TAG, "Updating Abs. Trip Path: " + absPath + " => " + relPath);
						...
						if (db.update(TripsTable.TABLE_NAME, tripValues, TripsTable.COLUMN_NAME + " = ?", new String[] {absPath}) == 0) {
							if (D) Log.e(TAG, "Trip Update Error Occured");
						}
					} 
				...
				if (receiptsCursor != null && receiptsCursor.moveToFirst()) {
					...
					do {
						...
						if (D) Log.d(TAG, "Updating Abs. Parent Path for Receipt" + id + ": " + absParentPath + " => " + relParentPath);;
						if (!absImgPath.equalsIgnoreCase(NO_DATA)) { //This can be either a path or NO_DATA
							...
							if (D) Log.d(TAG, "Updating Abs. Img Path for Receipt" + id + ": " + absImgPath + " => " + relImgPath);
						}
						if (db.update(ReceiptsTable.TABLE_NAME, receiptValues, ReceiptsTable.COLUMN_ID + " = ?", new String[] {Integer.toString(id)}) == 0) {
							if (D) Log.e(TAG, "Receipt Update Error Occured");
						}
					} 
	...
	public final void testPrintDBValues() {
		...
		final Cursor tripsCursor = db.query(TripsTable.TABLE_NAME, new String[] {TripsTable.COLUMN_NAME}, null, null, null, null, null);
		String data = "";
		if (D) Log.d(TAG, "=================== Printing Trips ===================");
		if (D) data += "=================== Printing Trips ===================" + "\n";
		if (tripsCursor != null && tripsCursor.moveToFirst()) {
			final int nameIndex = tripsCursor.getColumnIndex(TripsTable.COLUMN_NAME);
			do {
				if (D) Log.d(TAG, tripsCursor.getString(nameIndex));
				if (D) data += "\"" + tripsCursor.getString(nameIndex) + "\"";
		...
		final Cursor receiptsCursor = db.query(ReceiptsTable.TABLE_NAME, new String[] {ReceiptsTable.COLUMN_ID, ReceiptsTable.COLUMN_PARENT, ReceiptsTable.COLUMN_PATH}, null, null, null, null, null);
		if (D) Log.d(TAG, "=================== Printing Receipts ===================");
		if (D) data +=  "=================== Printing Receipts ===================" + "\n";
		if (receiptsCursor != null && receiptsCursor.moveToFirst()) {
			final int idIdx = receiptsCursor.getColumnIndex(ReceiptsTable.COLUMN_ID);
			final int parentIdx = receiptsCursor.getColumnIndex(ReceiptsTable.COLUMN_PARENT);
			final int imgIdx = receiptsCursor.getColumnIndex(ReceiptsTable.COLUMN_PATH);
			do {
				if (D) Log.d(TAG, "(" + receiptsCursor.getInt(idIdx) + ", " + receiptsCursor.getString(parentIdx) + ", " + receiptsCursor.getString(imgIdx) + ")");
				if (D) data += "(" + receiptsCursor.getInt(idIdx) + ", " + receiptsCursor.getString(parentIdx) + ", " + receiptsCursor.getString(imgIdx) + ")" + "\n";
			} 
			while (receiptsCursor.moveToNext());
		}
	...
	public void getTripsParallel() {
		if (mTripRowListener == null) {
			if (D) Log.d(TAG, "No TripRowListener was registered.");
		}
		else {
			synchronized (mTripCacheLock) {
				if (_areTripsValid) {
			...
			if (c != null && c.moveToFirst()) {
				...
				do {
					...
					if (qc != null && qc.moveToFirst() && qc.getColumnCount() > 0) {
						...
						else if (cnt == 0) curr = _activity.getPreferences().getDefaultCurreny();
						qc.close();
					}
					trips[c.getPosition()] = new TripRow(name, _activity.getStorageManager().getFile(name), from, to, price, curr, miles);
				} 
				while (c.moveToNext());
	...
	public final TripRow getTripByName(final String name) {
		...
		synchronized (mTripCacheLock) {
			if (_areTripsValid) {
				for(int i=0; i < _tripsCache.length; i++) {
					if (_tripsCache[i].dir.getName().equals(name))
		...
		if (c != null && c.moveToFirst()) {
			...
			c.close(); //Be sure to close the cursor to avoid memory leaks
			return new TripRow(name, _activity.getStorageManager().getFile(name), from, to, price, curr, miles);
		}
		else {
			c.close(); //Be sure to close the cursor to avoid memory leaks
			return null;
	...
	private TripRow insertTripHelper(final File dir, final Date from, final Date to) throws SQLException {
		...
		synchronized (mDatabaseLock) {
			if (values == null || db.insertOrThrow(TripsTable.TABLE_NAME, null, values) == -1)
				...
				return new TripRow(dir, from, to, _activity.getPreferences().getDefaultCurreny());
			}
		}
	}
	...
	private TripRow updateTripHelper(TripRow oldTrip, File dir, Date from, Date to) {
		try {
			...
			values.put(TripsTable.COLUMN_TO, to.getTime());
			synchronized (mDatabaseLock) {
				if (values == null || (db.update(TripsTable.TABLE_NAME, values, TripsTable.COLUMN_NAME + " = ?", new String[] {oldTrip.dir.getName()}) == 0))
					return null;
				else {
					if (!oldTrip.name.equalsIgnoreCase(dir.getName())) {
						synchronized (mReceiptCacheLock) {
							if (_receiptMapCache.containsKey(oldTrip)) _receiptMapCache.remove(oldTrip);
						}
						String oldName = oldTrip.name;
	...
	private boolean deleteTripHelper(TripRow trip) {
		...
		synchronized (mDatabaseLock) {
			success = (db.delete(ReceiptsTable.TABLE_NAME, ReceiptsTable.COLUMN_PARENT + " = ?", new String[] {trip.name}) >= 0);
		}
		if (success) {
			synchronized (mReceiptCacheLock) {
		...
		synchronized (mDatabaseLock) {
			success = (db.delete(TripsTable.TABLE_NAME, TripsTable.COLUMN_NAME + " = ?", new String[] {trip.name}) > 0);
		}
		return success;
	}
	...
	public final boolean addMiles(final TripRow trip, final String current, final String delta) {
		try {
			...
			trip.miles = total;
			return (db.update(TripsTable.TABLE_NAME, values, TripsTable.COLUMN_NAME + " = ?", new String[] {trip.name}) > 0);
		}
	...
	private final void updateTripPrice(final TripRow trip) {
		...
		final Cursor c = db.query(ReceiptsTable.TABLE_NAME, new String[] {"SUM(" + ReceiptsTable.COLUMN_PRICE + ")"}, 
				ReceiptsTable.COLUMN_PARENT + "= ? AND " + ReceiptsTable.COLUMN_EXPENSEABLE + " = 1", new String[] {trip.name}, null, null, null);
		if (c != null && c.moveToFirst() && c.getColumnCount() > 0) {
			...
			db.update(TripsTable.TABLE_NAME, values, TripsTable.COLUMN_NAME + " = ?", new String[] {trip.name});
		}
	}
	...
	public void getReceiptsParallel(final TripRow trip) {
		if (mReceiptRowListener == null) {
			if (D) Log.d(TAG, "No ReceiptRowListener was registered.");
		}
		synchronized (mReceiptCacheLock) {
			if (_receiptMapCache.containsKey(trip)) { //only cache the default way (otherwise we get into issues with asc v desc)
				if (mReceiptRowListener != null)
	...
	private final ReceiptRow[] getReceiptsHelper(final TripRow trip, final boolean desc) {
		...
		synchronized (mDatabaseLock) {
			...
			final Cursor c = db.query(ReceiptsTable.TABLE_NAME, null, 
					ReceiptsTable.COLUMN_PARENT + "= ?", new String[] {trip.name}, null, null, ReceiptsTable.COLUMN_DATE + ((desc)?" DESC":" ASC"));
			if (c != null && c.moveToFirst()) {
				...
				do {
					...
					if (!path.equalsIgnoreCase(DatabaseHelper.NO_DATA))
						img = _activity.getStorageManager().getFile(trip.dir, path);
					receipts[c.getPosition()] = new ReceiptRow(id, img, name, category, date, comment, price, tax, expensable, currency, fullpage, extra_edittext_1, extra_edittext_2, extra_edittext_3);
				} 
				while (c.moveToNext());
				c.close(); //Be sure to close the cursor to avoid memory leaks
			}
	...
	public final ReceiptRow getReceiptByID(final int id) {
		...
		if (c != null && c.moveToFirst()) {
			...
			File img = null;
			if (!path.equalsIgnoreCase(DatabaseHelper.NO_DATA))
				img = _activity.getStorageManager().getFile(_activity.getStorageManager().getFile(parent), path);
			c.close(); //Be sure to close the cursor to avoid memory leaks
			return new ReceiptRow(id, img, name, category, date, comment, price, tax, expensable, currency, fullpage, extra_edittext_1, extra_edittext_2, extra_edittext_3);
		}
		else {
			c.close(); //Be sure to close the cursor to avoid memory leaks
			return null;
	...
	private ReceiptRow insertReceiptHelper(TripRow trip, File img, String name, String category, Date date, 
		...
		values.put(ReceiptsTable.COLUMN_PARENT, trip.name);
		if (name.length() > 0)
		...
		synchronized (mDatabaseLock) {
			if (db.insertOrThrow(ReceiptsTable.TABLE_NAME, null, values) == -1)
				...
				if (c != null && c.moveToFirst() && c.getColumnCount() > 0) {
					...
					insertReceipt = new ReceiptRow(id, img, name, category, date, comment, price, tax, expensable, currency, fullpage, extra_edittext_1, extra_edittext_2, extra_edittext_3);
				}
				else {
					c.close();
					insertReceipt = null;
		...
		try {
			...
			synchronized (mDatabaseLock) {
				...
				if (values == null || (db.update(ReceiptsTable.TABLE_NAME, values, ReceiptsTable.COLUMN_ID + " = ?", new String[] {Integer.toString(oldReceipt.id)}) == 0))
					...
					this.updateTripPrice(trip);
					updatedReceipt = new ReceiptRow(oldReceipt.id, oldReceipt.img, name, category, date, comment, price, tax, expensable, currency, fullpage, extra_edittext_1, extra_edittext_2, extra_edittext_3);
				}
			}
			synchronized (mReceiptCacheLock) {
	...
	private boolean deleteReceiptHelper(ReceiptRow receipt, TripRow currentTrip) {
		...
		synchronized (mDatabaseLock) {
			final SQLiteDatabase db = this.getWritableDatabase();
			success = (db.delete(ReceiptsTable.TABLE_NAME, ReceiptsTable.COLUMN_ID + " = ?", new String[] {Integer.toString(receipt.id)}) > 0);
		}
		if (success) {
	...
	final synchronized boolean merge(String dbPath, String packageName, boolean overwrite) {
		...
		try {
			if (dbPath == null)
				return false;
			final SQLiteDatabase currDB = this.getReadableDatabase();
			importDB = SQLiteDatabase.openDatabase(dbPath, null, SQLiteDatabase.OPEN_READWRITE);
			Cursor c;
			//Merge Trips
			if(D) Log.d(TAG, "Merging Trips");
			...
			if (c != null && c.moveToFirst()) {
				...
				if (c != null) c.close(); //Be sure to close the cursor to avoid memory leaks
				importDB.close();
				return false;
			}
			
			//Merge Receipts
			if(D) Log.d(TAG, "Merging Receipts");
			...
			if (c != null && c.moveToFirst()) {
				...
				do {
					...
					final String extra_edittext_2 = c.getString(extra_edittext_2_Index);
					final String extra_edittext_3 = c.getString(extra_edittext_3_Index);
					final Cursor countCursor = currDB.rawQuery(queryCount, new String[] {newPath, name, Long.toString(date)});
					...
					if (countCursor != null) countCursor.close(); //Be sure to close the cursor to avoid memory leaks
				} 
				while (c.moveToNext());
				c.close(); //Be sure to close the cursor to avoid memory leaks
			}
			...
			if(D) Log.d(TAG, "Merging Categories");
