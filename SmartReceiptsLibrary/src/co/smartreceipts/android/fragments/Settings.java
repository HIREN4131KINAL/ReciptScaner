...
public class Settings implements ExportTask.Listener {
	...
	public void showSettingsMenu(final Context context) {
		...
		if (preferences.getMinimumReceiptPriceToIncludeInReports() != -Float.MAX_VALUE)
			minPrice.setText(Float.toString(preferences.getMinimumReceiptPriceToIncludeInReports()));
		...
		currencySpinner.setPrompt(mApp.getString(R.string.default_currency));
		...
		dateSeparatorSpinner.setAdapter(dateSeparators);
		dateSeparatorSpinner.setPrompt(mApp.getString(R.string.default_currency));
		int idx2 = dateSeparators.getPosition(defaultSepartor);
		if (idx2 > 0)
			dateSeparatorSpinner.setSelection(idx2);

		builder.setTitle(R.string.menu_main_settings).setView(scrollView).setCancelable(true).setPositiveButton(R.string.save, new DialogInterface.OnClickListener() {
	...
	public void showCategoriesMenu(final Context context) {
		...
		categoriesSpinner.setPrompt(mApp.getString(R.string.category));
		outerLayout.addView(categoriesSpinner);
		builder.setTitle(R.string.dialog_category_select).setView(outerLayout).setCancelable(true).setLongLivedPositiveButton(mApp.getString(R.string.add), new LongLivedOnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				final TextView nameLabel = new TextView(mApp.getCurrentActivity());
				nameLabel.setText(R.string.category_name);
				final EditText nameBox = new EditText(mApp.getCurrentActivity());
				final TextView codeLabel = new TextView(mApp.getCurrentActivity());
				codeLabel.setText(R.string.category_code);
				final EditText codeBox = new EditText(mApp.getCurrentActivity());
				layout.addView(nameLabel);
				layout.addView(nameBox);
				layout.addView(codeLabel);
				layout.addView(codeBox);
				innerBuilder.setTitle(mApp.getString(R.string.dialog_category_add)).setView(layout).setCancelable(true).setPositiveButton(R.string.add, new DialogInterface.OnClickListener() {
					@Override
			...
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				layout.addView(codeLabel);
				layout.addView(codeBox);
				innerBuilder.setTitle(mApp.getString(R.string.dialog_category_edit)).setView(layout).setCancelable(true).setPositiveButton(R.string.update, new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
			...
			public void onClick(DialogInterface dialog, int whichButton) {
				if (categoriesSpinner.getSelectedItem() == null) // There are no categories left to delete
					dialog.cancel();
				final BetterDialogBuilder innerBuilder = new BetterDialogBuilder(mApp.getCurrentActivity());
				innerBuilder.setTitle(mApp.getString(R.string.delete_item, categoriesSpinner.getSelectedItem().toString())).setCancelable(true).setPositiveButton(mApp.getString(R.string.delete), new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
	...
	public void showAbout(Context context) {
		...
		final BetterDialogBuilder builder = new BetterDialogBuilder(mApp.getCurrentActivity());
		builder.setTitle(mFlex.getString(context, R.string.DIALOG_ABOUT_TITLE)).setMessage(about).setCancelable(true).show();
	}
	...
	private void showCustomColumnMenu(final Columns columns, View checkBox, int titleStringId, final boolean isCSV) {
		...
		builder.setTitle(titleStringId).setView(parent).setCancelable(true).setLongLivedPositiveButton(mApp.getString(R.string.dialog_custom_csv_positive), new LongLivedOnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				if (isCSV) {
	...
	private class ColumnSelectionListener implements OnItemSelectedListener {
		...
		private int _index;
		private boolean _firstCall; // During the Spinner Creation, onItemSelected() is automatically called. This
									// boolean ignores the initial call
		private boolean _isCSV;

		public ColumnSelectionListener(DatabaseHelper db, int index, boolean isCSV) {
			_db = db;
			_index = index;
			_firstCall = true;
			_isCSV = isCSV;
